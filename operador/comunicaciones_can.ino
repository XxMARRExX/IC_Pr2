...
void send_sensor_status(Sensor& sensor){
  ...
  if (!sendPacketWithAck(packet, CAN_COMMAND_ID, 0)) return;
  ...
  if (!sendPacketWithAck(packet, CAN_COMMAND_ID, 1)) return;
  ...
  if (!sendPacketWithAck(packet, CAN_COMMAND_ID, 2)) return;
  ...
  for (int i = 0; i < nameLength; i += 7) {
    ...
    if (!sendPacketWithAck(packet, CAN_COMMAND_ID, packet[0])) return;
  }
  ...
  sendPacketWithAck(packet, CAN_COMMAND_ID, 255);
}


bool sendPacketWithAck(byte* packet, unsigned long id, byte expectedAck) {
  ...
  while (attempt < maxRetries) {
    if (CAN.sendMsgBuf(id, 0, 8, packet) == CAN_OK) {
        if (waitForAck(expectedAck)) {
            return true; // Paquete enviado y ACK recibido
        } else {
            Serial.print("Intento fallido, retransmitiendo paquete... Intento: ");
  ...
  Serial.println("Error: Se alcanzó el límite de intentos para el paquete.");
  return false; // Falló después de 10 intentos
}
